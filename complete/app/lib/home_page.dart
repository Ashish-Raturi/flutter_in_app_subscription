...
import 'package:flutter_in_app_purchases/subscription_db_service.dart';
...
String _kSilverSubscriptionId =
    Platform.isAndroid ? 'sliver_subscription' : 'your_ios_sub1_id';

String _kGoldSubscriptionId =
    Platform.isAndroid ? 'gold_subscription' : 'your_ios_sub2_id';
...
class _HomepageState extends State<Homepage> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<PurchaseDetails?>(
        stream: SubscriptionDbService().fetchOldSubscriptionDetails,
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            PurchaseDetails purchaseDetails = snapshot.data!;
            checkSubStatus(purchaseDetails);
            // _purchases = [snapshot.data!];
          }

          return Scaffold(
            appBar: AppBar(
              title: const Text('Flutter UPDATED Subscription App'),
            ),
            body: Stack(
              children: stack,
            ),
  ...
  checkSubStatus(PurchaseDetails purchaseDetails) async {
    bool res = await SubscriptionDbService().queryUserSubscriptionStatus(
    ...
    if (res) {
      ...
      _purchases = [];
      print('previous purchase not found');
    }
  }
  ...
  void deliverProduct(PurchaseDetails purchaseDetails) async {
    ...
    if (res != null && res.data.toString() == 'true') {
      setState(() {
